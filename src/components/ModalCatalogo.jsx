...
export default function ModalCatalogo({ onCerrar, onAgregar }) {
  ...
  const eliminar = async (it) => {
    const ok = await confirmar({
      titulo: 'Eliminar del catálogo', mensaje: `"${it.descripcion}"`,
  ...
  const usar = async (it) => {
    onAgregar({ cantidad: 1, descripcion: it.descripcion, precio: it.precio });
    supabase.from('servicios').update({ veces_usado: (it.veces_usado || 0) + 1 }).eq('id', it.id).then(() => {});
    toast.ok(`"${it.descripcion.slice(0, 30)}..." agregado.`);
  };

  const editar = (it) => {
    setEditId(it.id);
    setForm({ descripcion: it.descripcion, precio: it.precio, unidad: it.unidad || 'pza', categoria: it.categoria || 'General' });
    setMostrarForm(true);
  };

  const cats = useMemo(() => ['todas', ...new Set(items.map(i => i.categoria || 'General'))], [items]);

  const filtrados = useMemo(() => {
    const q = norm(busqueda);
    return items.filter(i =>
      (cat === 'todas' || (i.categoria || 'General') === cat) &&
      (!q || norm(i.descripcion).includes(q))
    );
  }, [items, busqueda, cat]);
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="p-5 sm:p-6 bg-white sm:rounded-t-3xl rounded-t-3xl border-b border-slate-200 shrink-0 space-y-4">
          <div className="flex justify-between items-start gap-3">
            ...
            <button onClick={onCerrar} className="p-2 text-slate-300 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition">
              <X size={20} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
            ...
            <form onSubmit={guardar} className="bg-white p-5 rounded-3xl border-2 border-primario shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="relative">
                  ...
                  <input type="number" inputMode="decimal" min="0" step="any" value={form.precio}
                    onChange={(e) => setForm({ ...form, precio: e.target.value })} onFocus={(e) => e.target.select()}
          ...
          {cargando ? (
            <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-16 bg-slate-200 rounded-2xl animate-pulse" />)}</div>
          ) : filtrados.length === 0 ? (
            ...
            <div className="space-y-2">
              {filtrados.map(it => (
                <div key={it.id}
                  className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center gap-3 group hover:border-primario hover:shadow-sm transition">
